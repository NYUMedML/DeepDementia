...
def main():
    ...
    if cfg['data']['raw_data']:
        # train_dataset = ADNI_3D_numpy(dataset = cfg['data']['dataset'], mode = 'Train', 
        #     n_label = cfg['model']['n_label'], percentage_usage=cfg['data']['percentage_usage'])
        # val_dataset = ADNI_3D_numpy(dataset = cfg['data']['dataset'], mode = 'Val', n_label = cfg['model']['n_label'])

        train_dataset = ADNI_3D_raw(dir_to_scans, dir_to_tsv, mode = 'Train', 
            n_label = cfg['model']['n_label'], percentage_usage=cfg['data']['percentage_usage'])
    ...
    criterion = get_loss_criterion(cfg,type='CrossEntropyLoss',num_examp=len(train_dataset)).to(device)
...
def train(cfg, train_loader, main_model, scheduler, 
    ...
    logit_all = []
    target_all = []
    for i, (input, target, idx, index, mmse, segment, age) in enumerate(train_loader):
...
def validate(cfg,val_loader,main_model,criterion,epoch):
    ...
    for i, (input, target, index ,patient_idx, mmse, segment,age) in enumerate(val_loader):
        # measure data loading time
        data_time.update(time.time() - end)
        input = input.to(device)
        target = target.to(device)
