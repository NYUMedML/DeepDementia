...
class FullModel(nn.Module):
    ...
    def forward(self, inputs, targets):
        outputs = self.model(inputs[-2],inputs[-1])
        if len(inputs) > 2:
            outputs_u_w = self.model(inputs[1],None)
            outputs_u = self.model(inputs[2],None)
            loss = self.loss(outputs, outputs_u_w, outputs_u, targets)
        else:
...
class FullModel_semi(nn.Module):
    def __init__(self, model, loss):
        ...
        self.model = model
        self.loss =  loss
    def forward(self, epoch, inputs, targets):
        outputs = self.model(inputs[0],inputs[-1])
        if len(inputs) > 2:
            outputs_u_w = self.model(inputs[1],None)
            outputs_u = self.model(inputs[2],None)
            loss = self.loss(epoch, outputs, outputs_u_w, outputs_u, targets)
        else:
            loss = self.loss(epoch, outputs, None, None, targets)
        return torch.unsqueeze(loss,0),outputs


def DataParallel_withLoss_semi(model,loss, **kwargs):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    if torch.cuda.device_count()>1:
        print("lets use multiple gpu!",torch.cuda.device_count())
    model=FullModel_semi(model, loss)
    if 'device_ids' in kwargs.keys():
        device_ids=kwargs['device_ids']
    else:
        device_ids=None
...
class FullModel_contrast(nn.Module):
    def __init__(self, model, loss):
        super(FullModel_contrast, self).__init__()
        self.model = model
        self.loss =  loss
    def forward(self, epoch, inputs, targets):
        outputs, _ = self.model(inputs[0],inputs[-1])
        if len(inputs) > 2:
            _, z_i = self.model(inputs[1],None)
            _, z_j = self.model(inputs[2],None)
            loss = self.loss(epoch, outputs, z_i, z_j, targets)
        else:
            loss = self.loss(epoch, outputs, None, None, targets)
        return torch.unsqueeze(loss,0),outputs


def DataParallel_withLoss_contrast(model,loss, **kwargs):
    device = torch.device("cuda:0" if torch.cuda.is_available() else "cpu")
    if torch.cuda.device_count()>1:
        print("lets use multiple gpu!",torch.cuda.device_count())
    model=FullModel_contrast(model, loss)
    if 'device_ids' in kwargs.keys():
        device_ids=kwargs['device_ids']
    else:
        device_ids=None
...
def get_auc_data(logit_all, target_all,n_label):
    y = np.eye(n_label)[target_all].astype('int')
