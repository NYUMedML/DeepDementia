...
import numpy as np
import torchio
from datasets.adni_3d_raw import ADNI_3D_raw
from datasets.adni_3d import ADNI_3D
from datasets.adni_3d_numpy import ADNI_3D_numpy
from datasets.adni_3d_semi import get_adni_dataset,get_ukbiobank_dataset,get_aibl_dataset

from lib.Loss import get_loss_criterion
from lib.utils import DataParallel_withLoss_contrast, get_auc_data, accuracy, AverageMeter, balanced_accuracy_score, clip_gradients, visualize_visdom
#import warnings
import warnings
...
def main():
    ...
    train_dataset = get_adni_dataset(dir_to_scans, dir_to_tsv, mode = 'Train', 
        n_label = cfg['model']['n_label'], 
        percentage_usage=cfg['data']['percentage_usage'], 
        resample = cfg['data']['raw_data'])
    root_dir = cfg['data']['root_dir']

    if 'ukbiobank' in root_dir:
        train_dataset_unlabeled = get_ukbiobank_dataset('train', root_dir, ['T1'], 
            splitFile=os.path.join(root_dir,'brain_rep_data/train_ids.txt'))
    elif 'aibl' in root_dir:
        train_dataset_unlabeled = get_aibl_dataset(root_dir, dir_to_tsv, mode = 'All', 
                n_label = cfg['model']['n_label'], 
                percentage_usage=cfg['data']['percentage_usage'])
    val_dataset = get_adni_dataset(dir_to_scans, dir_to_tsv, mode = 'Val', 
        n_label = cfg['model']['n_label'], 
        resample = cfg['data']['raw_data'])
    ...
    train_loader_unlabeled = torch.utils.data.DataLoader(
        train_dataset_unlabeled, batch_size=cfg['data']['batch_size'], shuffle=True,
        num_workers=cfg['data']['workers'], pin_memory=True, drop_last=True)
    # train_dataset_unlabeled = [0]
    ...
    nudata = len(train_dataset_unlabeled)
    print('In total ', str(ndata), ' labeled scans and ',str(nudata), ' unlabeled scansin training set')
    ...
    criterion = get_loss_criterion(cfg,type='semi_loss_contrastive',num_examp=len(train_dataset)).to(device)
    criterion_val = get_loss_criterion(cfg,type='CrossEntropyLoss',num_examp=len(train_dataset)).to(device)

    #Loss parallel
    model = DataParallel_withLoss_contrast(main_model,criterion)
    ...
    for epoch in range(cfg['training_parameters']['start_epoch'], cfg['training_parameters']['epochs']):
        ...
        # train for one epoch
        train_loss, train_acc = train(cfg,train_loader, train_loader_unlabeled, model, scheduler, criterion, main_optim, epoch)
...
def train(cfg, train_loader, unlabeled_trainloader, main_model, scheduler, 
    ...
    for i, subjects_batch in enumerate(train_loader):
        input = subjects_batch['t1'][torchio.DATA]
        target = subjects_batch['label']
        age = subjects_batch['age']

        try:
            subjects_batch_u = unlabeled_train_iter.next()
            input_u1 = subjects_batch_u['t1'][torchio.DATA]
            input_u2 = subjects_batch_u['t2'][torchio.DATA]
        except:
            unlabeled_train_iter = iter(unlabeled_trainloader)
            subjects_batch_u = unlabeled_train_iter.next()
            input_u1 = subjects_batch_u['t1'][torchio.DATA]
        ...
        input_u1, input_u2 = input_u1.to(device), input_u2.to(device)
        target = target.to(device)


        main_loss, logit = main_model(epoch,[input, input_u1, input_u2, age], target)
...
def validate(cfg,val_loader,main_model,criterion,epoch):
    ...
    for i, subjects_batch in enumerate(val_loader):
        ...
        main_loss, logit = main_model(epoch, [input, age], target)
        logit = logit.data.detach().cpu()
        main_loss = main_loss.mean().cpu().item()

        
        # try:
        #     logit = logit.data.cpu()
        # except:
        #     pass
        logit_all.append(logit)
        target_all.append(torch.tensor(target.data.cpu()))
        acc,_ = accuracy(logit,target.data.cpu())
        _, preds = torch.max(logit, 1)
        for t, p in zip(target.cpu().view(-1), preds.view(-1)):
                confusion_matrix[t.long(), p.long()] += 1

        acc,correct = accuracy(logit,target.cpu())
        correct_all += correct[0].item()

        # measure accuracy and record loss
        main_losses.update(main_loss, input.size(0))
